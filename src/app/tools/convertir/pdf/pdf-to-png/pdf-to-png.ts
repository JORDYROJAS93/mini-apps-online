...
@Component({
  ...
  templateUrl: './pdf-to-png.html',
  styleUrls: ['./pdf-to-png.css'],
  encapsulation: ViewEncapsulation.None // Crucial para Tailwind
})
export class PdfToPngComponent implements OnInit {
  @ViewChild('fileDropZone') dropZoneRef!: ElementRef;
  @ViewChild('fileInput') fileInputRef!: ElementRef<HTMLInputElement>;
  
  readonly isDragging = signal(false);
  readonly quality = signal(1.5); 
  ...
  handleFile(file: File): void {
    ...
    this.converterService.convertPdfToPng(file, this.quality());
  }

  downloadImage(image: PageImage, index: number): void {
    const link = document.createElement('a');
    link.href = image.dataUrl;
    link.download = `${this.converterService.result()?.originalName}_pagina_${index + 1}.png`; // Descarga .png
    link.click();
  }
  ...
  async downloadAllAsZip(): Promise<void> {
    ...
    result.images.forEach((img, index) => {
      ...
      folder?.file(`pagina_${index + 1}.png`, base64Data, { base64: true });
    });

    const content = await zip.generateAsync({ type: 'blob' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(content);
    link.download = `${result.originalName}_png.zip`;
    link.click();
  }
